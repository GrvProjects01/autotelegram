...
async def lovable_request(
    ...
    try:
        ...
        print(f"[Lovable] HTTP {error.response.status_code}")
        print("[Lovable] URL:", url)
        try:
        ...
        print("[Lovable] Erro de conexão:", str(error))
        raise
...
def debug_automations(automations):
    print("\n================ DEBUG AUTOMAÇÕES ================")
    print("[DEBUG] Automações recebidas:", len(automations))
    ...
    for index, automation in enumerate(automations, start=1):
        ...
        print("[DEBUG] Source:", automation.get("source_chat_id"))
...
async def get_matching_automations(source_id):
    ...
    if EVENT_DEBUG:
        ...
        if not matches:
            print("[Match] Nenhuma automação para este chat.")
            print("[Match] Origens configuradas:", configured_sources)
...
async def send_heartbeat():
    ...
    if isinstance(response, dict):
        ...
        if account_id:
            print("[Heartbeat] Conta Lovable:", account_id)
...
async def warm_entity_cache():
    print("[Entities] Carregando entidades...")
    count = 0
    ...
    print(f"[Entities] {count} entidades carregadas.")
...
async def resolve_destination_entity(destination_chat_id):
    ...
    raise ValueError(f"Não foi possível resolver o chat {destination_str}")


async def sync_telegram_chats():
    print("[Chats] Sincronizando...")
    me = await client.get_me()
    chats = []
    ...
    print("[Chats] Sincronizados:", response.get("synced", len(chats)))
    chats.clear()
    gc.collect()
...
async def chat_sync_loop():
    while True:
        try:
            ...
            print("[Chats] erro:", type(error).__name__, str(error))
...
async def save_message_link(
    ...
    try:
        ...
        print("[Link] Persistido no Lovable.")
    except Exception as error:
        print(
            "[Link] Persistência remota falhou, cache local ativo:",
            type(error).__name__
        )
...
async def find_message_links(
    ...
    try:
        ...
        print("[Link] Busca remota falhou:", type(error).__name__)
        return []
    ...
    for key in MESSAGE_LINK_CACHE.keys():
        ...
        if key.startswith(prefix):
            if (
                ...
                or key.endswith(f":{automation_id}")
            ):
...
def adjust_entities_for_replacements(entities, occurrences, max_text_len=0):
    ...
    for occurrence in occurrences:
        replace_start = occurrence["start"]
        old_length = occurrence["old_length"]
        new_length = occurrence["new_length"]
        replace_end = replace_start + old_length
...
def process_hidden_urls(entities, replacements):
    result = copy.deepcopy(entities or [])

    for entity in result:
        if isinstance(entity, MessageEntityTextUrl):
            entity.url = replace_value(entity.url or "", replacements)

    return result


def check_blacklist(text, entities, automation):
    blacklist = automation.get("blacklist", []) or []
    hidden_urls = []
    ...
    for rule in blacklist:
        ...
        if match_type == "contains":
            detected = term.lower() in content.lower()
        elif match_type == "exact":
            detected = content.lower().strip() == term.lower().strip()
        elif match_type == "regex":
            try:
                detected = bool(re.search(term, content, flags=re.IGNORECASE))
            except re.error:
                continue

        if detected and rule.get("action", "block") == "block":
            return True

    return False


def process_rich_text(text, entities, automation):
    text = text or ""
    entities = copy.deepcopy(entities or [])

    if check_blacklist(text, entities, automation):
        return None, []
...
async def publish_single_message(
    ...
    claimed = await claim_processing_key(processing_key)
    if not claimed:
        return
    ...
    try:
        ...
        if sent:
            ...
            print(f"[Publish] Publicada: {message.id} → {sent.id} ({elapsed:.2f}s)")
...
async def album_handler(event):
    ...
    if getattr(event, "out", False):
        if await is_self_published_album(source_id, event.messages):
            return

    matches = await get_matching_automations(source_id)
    if not matches:
        return

    media_messages = [msg for msg in event.messages if msg.media]
    ...
    original_caption = caption_message.message or ""
    original_entities = caption_message.entities or []
        ...
        try:
            ...
            if not preserve_media:
                ...
                await save_message_link(
                    ...
                    source_chat_id=source_id,
                    source_message_id=caption_message.id,
                    source_grouped_id=event.grouped_id,
                    destination_chat_id=destination,
                    destination_message_id=sent.id
                )
                continue

            captions = ["" for _ in media_messages]
            album_entities = [[] for _ in media_messages]

            if preserve_caption:
                captions[caption_index] = processed_caption
                if preserve_formatting:
                    album_entities[caption_index] = processed_entities
            ...
            async with MEDIA_SEND_SEMAPHORE:
                ...
                try:
                    ...
                    print(
                        "[Protected Album] Álbum restrito detectado. "
                        "Baixando mídias para bypass..."
                    )
                    temp_files = []
                    try:
                        ...
                        sent_msg = await client.send_message(
                            ...
                            formatting_entities=(processed_entities if preserve_formatting else [])
                        )
                ...
                elapsed = time.monotonic() - started_at
                print(f"[Album] Publicado: {len(sent_messages)} itens ({elapsed:.2f}s)")

                for source_message, destination_message in zip(media_messages, sent_messages):
                    await remember_self_published(destination, destination_message.id)
                    await save_message_link(
                        automation_id=automation_id,
                        source_chat_id=source_id,
                ...
                await send_log(
                    ...
                    source_message_id=caption_message.id,
                    destination_message_id=(
                        sent_messages[0].id if sent_messages else None
                    ),
                    status="published",
                    original_text=original_caption,
...
async def new_message_handler(event):
    ...
    for automation in matches:
        try:
            await publish_single_message(message, source_id, automation)
        except Exception as error:
            print("[NewMessage] ERRO:", type(error).__name__, str(error))
...
async def edited_message_handler(event):
    ...
    for automation in matches:
        ...
        processed_text, processed_entities = process_rich_text(
            message.message or "",
            message.entities or [],
            automation
        )
        ...
        if message.media and not preserve_caption:
            continue
...
def save_source_recovery_state(state):
    ...
    temporary_path = SOURCE_RECOVERY_STATE_FILE + ".tmp"
    ...
    os.replace(temporary_path, SOURCE_RECOVERY_STATE_FILE)
...
async def recover_source_messages(source_text, last_message_id):
    source_id = source_id_value(source_text)
    source_entity = await resolve_destination_entity(source_id)
    ...
    recovered_messages.sort(key=lambda item: int(item.id))

    processed_albums = set()
    newest_message_id = int(last_message_id)

    for message in recovered_messages:
        newest_message_id = max(newest_message_id, int(message.id))
        ...
        if grouped_id:
            ...
            album_messages = [
                ...
                if getattr(album_message, "grouped_id", None) == grouped_id
            ]
            ...
            await album_handler(
                SimpleNamespace(
                    ...
                    out=any(bool(getattr(item, "out", False)) for item in album_messages)
                )
            )
            continue
...
async def source_recovery_loop():
    ...
    while True:
        try:
            ...
            print("[Recovery] ERRO:", type(error).__name__, str(error))

        await asyncio.sleep(max(5, SOURCE_RECOVERY_INTERVAL))
...
async def main():
    ...
    try:
        ...
        print("[Lovable] Automações ativas:", len(automations))
        debug_automations(automations)
    except Exception as error:
        print("[Lovable] ERRO:", type(error).__name__, str(error))
